...
Для небольшого объема задач: ставишь пакеты, крутишь ssh / curl / nginx / простые сервисы —5% вполне хватает.  
...
```
**Ответ**  
Создадим в корне проекта [vms_platform.tf](../vms_platform.tf). Сделаем перенос в него всех переменных. Создадим вторую виртуальную машину [в main.tf](../main.tf).  
